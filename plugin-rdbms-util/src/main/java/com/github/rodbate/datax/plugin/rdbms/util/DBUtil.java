package com.github.rodbate.datax.plugin.rdbms.util;
...
//                    throw new Exception(DBUtilErrorCode.JDBC_NULL.toString());
    ...
    private static synchronized Connection connect(DataBaseType dataBaseType,
        ...
        //ob10的处理
        if (url.startsWith(com.github.rodbate.datax.plugin.rdbms.writer.Constant.OB10_SPLIT_STRING) && dataBaseType == DataBaseType.MySql) {
            String[] ss = url.split(com.github.rodbate.datax.plugin.rdbms.writer.Constant.OB10_SPLIT_STRING_PATTERN);
            if (ss.length != 3) {
            ...
            return RetryUtil.executeWithRetry(() -> {
                ...
                try {
                    ...
                    throw DataXException.asDataXException(DBUtilErrorCode.RS_ASYNC_ERROR,
                        String.format("select DATABASE_ROLE from V$DATABpASE failed,请检查您的jdbcUrl:%s.", url));
                } finally {
