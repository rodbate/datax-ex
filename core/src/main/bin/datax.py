...
dataxServerAddress = "localhost:22000"
...
DEFAULT_PROPERTY_CONF = "-Dfile.encoding=UTF-8 -Dlogback.statusListenerClass=ch.qos.logback.core.status.NopStatusListener -Djava.security.egd=file:///dev/urandom -Ddatax.home=%s -Dlogback.configurationFile=%s -DdataxServerAddress=%s" % (
    DATAX_HOME, LOGBACK_FILE, dataxServerAddress)
ENGINE_COMMAND = "java -server ${jvm} %s -classpath %s  ${params} com.globalegrow.datax.core.Engine -mode ${mode} -jobid ${jobid} -job ${job}" % (
