package com.github.rodbate.datax.core.util;
    ...
    private static String getJobContent(String jobResource) {
        ...
        if (isJobResourceFromHttp) {
            ...
            try {
                ...
                String resp = httpClientUtil.executeAndGetWithFailedRetry(httpGet, 1, 1000L);
                WebResponse<GetJobConfigResp> response = JSON.parseObject(resp, new TypeReference<WebResponse<GetJobConfigResp>>() {
                });
                if (response.getCode() != ReturnCode.OK.getCode()) {
                    throw new RuntimeException(String.format("msg: %s, error: %s", response.getMsg(), response.getException()));
                }
                jobContent = response.getData().getJobConf();
            } catch (Exception e) {
